...
function validateMessage() {
	...
	if((msj.value.trim().length == 0) || (msj.value.trim().length > 128)) {
		alert("El mensaje no puede estar vacío y debe tener menos de 128 caracteres")
		msj.focus()
		retorno = false
	}
...
function ocultar(){
    document.getElementById('mensages').style.display = 'none';
}
function mostrar(){
    document.getElementById('mensages').style.display = 'inline';
}
function ocultar2(){
...
function validateMessageDHTML() {
	...
	if((msj.value.trim().length == 0) || (msj.value.trim().length > 128)) {
		errores += "El mensaje no puede estar vacío y debe tener menos de 128 caracteres"
    ...
    $.getJSON("/friends",
        function(data) {
            ...
            $('#friends').append("<div class='par' id='t0'>" +
                    "<input id='check0' value='All' type='checkbox' onclick='pulsadoCheck(0)' style='margin:10px' />All</div>")
            data.forEach(function(item) {
                $('#friends').append("<div class='" + ((i%2) ? "impar" : "par") + "' id='t" + i + "'>" +
                    "<input id='check" + i + "' value='" + item + "' type='checkbox' onclick='pulsadoCheck(" + i + ")' style='margin:10px' />" +
                    item + "</div>")
                i++
            });
            $("#div_friends").show()
...
function selectFriend() {
 if(seleccionados.length > 0) {
        ...
        alert("Por favor, seleciona a un amigo")
    }
}