...
def home():
    ...
    if 'user_name' not in session:
        return "ERROR: Debe iniciar sesión antes de usar la aplicación"
    ...
    return render_template('home.html', logged=True, nickname=session['user_name'], messages=messages,
                           friends=session['friends'], selected_friend=request.form.get('friend', 'All'), photo=session['photo'], all_images=get_all_images(), date_birth=session['date_birth'])
...
def profile():
    ...
    """
    if 'user_name' not in session:
        return process_error("Debe registrarse antes de usar la aplicacion",
                             url_for("login"))
    if request.method == 'POST':
        session['user_name'] = request.form['nickname']
        session['password'] = request.form['passwd']
        session['date_birth'] = request.form['date_birth']
        session['photo'] = request.form['photo']
        session['friends'] = [str.strip(str(friend)) for friend in request.form.getlist('friends')]
        return redirect(url_for("profile"))
    else:  # The http GET method was used
        return render_template("edit_profile.html", nickname=session['user_name'], email=session['email'], date_birth=session['date_birth'], photo=session['photo'],
                               passwd=session['password'], friends=session['friends'],
                               all_users=get_all_users(session['email']), all_photo=get_all_photo())
...
def process_signup():
    faltan = []
    campos = ['nickname', 'email', 'date_birth', 'photo' ,'passwd', 'confirm', 'signup_submit']
    for campo in campos:
        value = request.form.get(campo, None)
        if value is None or value == '':
            faltan.append(campo)
    return create_user_file(request.form['nickname'], request.form['email'], request.form['date_birth'], request.form['photo'], request.form['passwd'], request.form['confirm'])
...
def load_user(email, passwd):
    ...
    session['date_birth'] = data['date_birth']
...
def create_user_file(name, email, dateb, pht, passwd, passwd_confirmation):
    """
...
def get_photo_friend():

    photo_friend = ""
    for friend in session['friends']:
        texts = load_photo_from_user(friend)
        photo_friend = texts
    return photo_friend

def load_photo_from_user(user):
    """
    Get all the message stored for the given user
    :param user: the user whose message will be returned
    :return: all the message published by the given user as a list of (user, time stamp, message)
...
def get_all_photo():
    dir_path = os.path.join(SITE_ROOT, "static/css/img_avatar/")
    user_list = listdir(dir_path)
    return user_list

def get_all_images():
    dir_path = os.path.join(SITE_ROOT, "static/css/img_post/")
    user_list = listdir(dir_path)
    return user_list
